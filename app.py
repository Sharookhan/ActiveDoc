...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://activedoc_user:koVVT3TmbDDDt4AdHER8TLwgDt1Pe6OG@dpg-clivvghll56s73fc2qo0-a.oregon-postgres.render.com/activedoc'
...
@app.route('/logout')
def logout():
    exists = db.session.query(logindetails).filter_by(
...
def login():
    ...
    if request.method == 'POST':
        ...
        exists = None
        exists = db.session.query(logindetails).filter(
            logindetails.username == uname, logindetails.password == sha256(password1.encode()).hexdigest()).first()
        if exists:
    ...
    if request.method == 'POST':
        ...
        if password != confirmpassword:
            ...
            exists = db.session.query(db.session.query(
                userdetails).filter_by(username=username).exists()).scalar()
            if exists == True:
                error = 'Username already Exit'
                return render_template('signup.html', error=error)

            exists = db.session.query(db.session.query(
                userdetails).filter_by(email=email).exists()).scalar()
            if exists == True:
                ...
                now2 = datetime.timestamp(now)
                dnow = datetime.fromtimestamp(now2)
                login = logindetails(username, sha256(
                    password.encode()).hexdigest(), dnow, False)
...
def active_users():
    emoji = ["far fa-grin-beam-sweat", "far fa-smile-wink",
             "far fa-laugh-squint", "far fa-kiss-beam", "far fa-grin-hearts"]
    color = ["text-primary", "text-warning",
             "text-dark", "text-primary", "text-success"]
    result = " "
    j = 0
    active = db.session.query(
        logindetails.username).filter_by(activestatus=True)
    if active:
        for i in active:
            uactive = db.session.query(userdetails).filter(
                userdetails.username == i[0]).first()
            str1 = '''
